...
1. **用户数据是模糊转储，不是字段映射。**
   `parseUserData` 的采样方式是「读 `CRoomUserModel` 固定长度字节窗口 + 抽可打印 ASCII」。
   能得到身份字符串（id / 人名 / 国家 / 州）与数值，足以**区分填充记录和像真人的记录**；
   但**拿不到结构化字段定义**（字段类型/偏移语义）。
   → 因此本报告只能说「存在填充特征」，不能说「已解析出 XX 字段」。

2. **样本量与采样窗口有限。**
   本轮采到 29 条用户记录（部分统计以 28 为分母，差异已在正文标注），
   且采样窗口受玩家在大厅的停留行为限制。
   → **真人/填充的比例未被量化**；不要把「已确认填充特征」写成「机器人占比 X%」。

3. **未覆盖项**：完整行为循环（从站到离座的整轮）、大厅与机台内一致性、
   中奖表现。这三项需要更长的采集窗口或录屏补充。

4. **仓库版本与首次交付版本存在时间差**（评审指出的"脚本缺失"问题）：
   脚本最初只在本机，`06f1d1e` 时期确实不在树里；09-15 起已入库到
   `tools/analysis/popslots/`，因此该缺口已从"等外部提供"变为"本机可复现"。

## 复现工具（已入库）

`tools/analysis/popslots/` —— 公共层 `pop_common.py`（adb/PID/输出/符号名全部可配置，
符号在 attach 时按名字解析，ASLR 安全）+ 12 个脚本，用法见该目录 README。
