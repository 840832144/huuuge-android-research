...
> ⚠️ **iOS 26 特有注意**（用户实测 26.6.1）：
> - 装描述文件在 `设置→通用→VPN与设备管理`（iOS 26 名称可能显示为 "VPN 与设备管理"）。
> - **必须**到 `设置→通用→关于本机→证书信任设置` 里**手动打开** mitmproxy CA 的信任开关，
>   否则 iOS 不信任、抓不到。iOS 26 这一步**没有自动信任**，一定要手动开。
> - 若证书信任设置里**没出现** mitmproxy CA → 一般是描述文件没装成，先回 VPN与设备管理确认。
