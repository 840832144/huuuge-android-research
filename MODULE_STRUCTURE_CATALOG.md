...
The first catalog baseline is generated by `scripts/build_module_catalog.py` under `artifacts/module_catalog/`.

It currently covers all 36 descriptor files, all 1028 descriptor message types and all 356 `Services.proto` methods across 37 module dossiers. The baseline combines the sanitized `20260825_182300` inventory with local-only populated-field variability counts and base-APK ZPK filenames; no captured values are committed.

Future captures should rerun the builder and update the same dossiers rather than creating parallel one-off analyses.
