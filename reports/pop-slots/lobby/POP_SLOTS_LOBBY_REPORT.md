...
> **路径更新（2026-09-17）**：上文与附录 B 引用的 `artifacts/toptycoon/POP_SLOTS_LOBBY_FORENSICS.md` 在本报告写成时**是正确的**；该文件随后在 main 的 `041f014`（`R100`）中移入 `artifacts/popslots/POP_SLOTS_LOBBY_FORENSICS.md`，并新增 `artifacts/popslots/ENVIRONMENT_LOCK.md`。这是仓库重组，**不是报告写错**。同时 `tools/analysis/popslots/` 已入库（13 个脚本：公共层 `pop_common.py` + 12 个分析脚本），因此附录 B 的「Hook 脚本缺口」已从**等外部提供**变为**本机可复现**。
...
| 角色行为层 | §1.2：Walk、Stand、WalkToSit、活动工厂、生命周期、`CSlotsFinder::sitUser` | 原报告记载；视频另证有站立/移动/在座 | 完整状态转换日志及角色对应；找席函数名不证明目标选择策略 [E17] |
...
|---|---|---|---|
| 机器人身份与旧采样口径 | 优先由原研究者提供现有脱敏 Hook 摘要、字段/去重说明及判断依据（**注：Hook 脚本已于 2026-09 入库 `tools/analysis/popslots/`，符号按名字解析、PID 与输出目录均可配置，可在有 root 的研究实例上复跑采样 —— 已不是外部依赖**） | 样本—角色—自动控制证据之间可追溯；解释 29/28 | 继续保留 DSH 高概率判断，不制造确定标签 |
